...
const gapHistoryLabel = (operation: string) => ({ EDIT: '수정', SET_DISMISSED: '제외', SET_RESOLVED: '확인' }[operation] ?? '변경');
...
export const ContextWorkspace: React.FC<{ caseId: string; refreshKey: string; institutions: React.ReactNode; onOpenParticipants: () => void }> = ({ caseId, refreshKey, institutions, onOpenParticipants }) => {
  ...
  const addTask = () => start(titleCommand('task-new', '담당자 업무 추가', 'tasks', { client_request_id: generateUuid(), task_type: 'OTHER', priority: 'NORMAL' }));
  ...
  const suggestion = (id: string, title: string, description: string, version: number, legacy = false) => <article className="context-resource ai" key={id}><b>{userText(title)}</b><p>{userText(description)}</p><div className="context-resource-actions">
    <button disabled={busy || !data?.can_review_suggestions} onClick={() => start(titleCommand(id, '내용 검토 후 업무로 채택', `${legacy ? 'legacy-suggestions' : 'suggestions'}/${id}/review`, { expected_version: version, decision: 'ACCEPT' }, title, description, legacy ? 'POST' : 'PATCH', 'edited_title', 'edited_description'))}><Check size={13}/><Pencil size={13}/>수정·채택</button>
    <button disabled={busy || !data?.can_review_suggestions} onClick={() => start(noteCommand(id, '제안 제외', `${legacy ? 'legacy-suggestions' : 'suggestions'}/${id}/review`, { expected_version: version, decision: 'DISMISS' }, legacy ? 'POST' : 'PATCH'))}><X size={13}/>제외</button>
    </div>{form(id)}</article>;
  return <>
    ...
    {!data ? <p className="context-empty">사실·업무 기록을 불러오는 중입니다.</p> : <>
      {!data.can_write && <div className="permission-notice"><strong>읽기 전용 사건 정보</strong><p>현재 역할에서는 사건 정보와 대응 업무를 조회할 수 있습니다.</p><button className="secondary-action" type="button" onClick={onOpenParticipants}>참여자 관리에서 역할 확인</button></div>}
      <section className="context-section"><h3>분석 정황</h3><p className="context-section-description">AI 분석·대화·담당자 입력을 바탕으로 정리된 준사실입니다. 필요한 내용은 질문과 확인 업무로 이어서 점검할 수 있습니다.</p>
        {!(data.context_facts ?? data.proposed_facts).length && <Empty/>}
        {(data.context_facts ?? data.proposed_facts).map(f => <article className="context-resource" key={f.fact_id}><b>{userText(f.display_label)}</b><p>{userText(f.display_value)}</p><small>{f.source_kind === 'AI_EXTRACTION' ? 'AI 분석 정황' : f.source_kind === 'CUSTOMER_STATEMENT' ? '고객 발언' : f.source_kind === 'STAFF_OBSERVATION' ? '담당자 입력' : '연결된 기록'} · 근거 {f.evidence_refs?.length ?? 0}건</small></article>)}
        <button className="secondary-action" disabled={busy || !data.can_write} onClick={() => start(titleCommand('fact-new', '분석 정황 추가', 'facts', { client_request_id: generateUuid(), semantic_key: `staff.observation.${generateUuid().replace(/-/g, '')}`, value: {} }, '', '', 'POST', 'display_label', 'display_value'))}><Plus size={13}/>정황 추가</button>{form('fact-new')}
      </section>
      <section className="context-section"><h3>추가 확인이 필요한 내용</h3><p className="context-section-description">분석 정황만으로 판단하기 어려워 질문·기관 확인·담당자 업무가 필요한 내용입니다.</p>
        {!data.open_gaps.length && !data.legacy_gaps.length && <Empty/>}
        {data.open_gaps.map(g => <article className="context-resource" key={g.gap_id}><header><b>{userText(g.title)}</b><small>{states[g.status]}</small></header><p>{userText(g.reason)}</p><div className="context-resource-actions"><button disabled={busy || !data.can_write} onClick={() => start(titleCommand(`gap-${g.gap_id}`, '추가 확인 내용 수정', `gaps/${g.gap_id}`, { expected_version: g.version }, g.title, g.reason, 'PATCH', 'edited_title', 'edited_reason'))}><Pencil size={13}/>수정</button><button disabled={busy || !data.can_write} onClick={() => start(noteCommand(`gap-${g.gap_id}`, '확인 항목 제외', `gaps/${g.gap_id}`, { expected_version: g.version, status: 'DISMISSED' }))}><X size={13}/>제외</button></div>{form(`gap-${g.gap_id}`)}</article>)}
        {data.legacy_gaps.map(g => <article className="context-resource" key={g.id}><header><b>{userText(g.title)}</b><small>{states[g.status]}</small></header><p>{userText(g.reason)}</p><div className="context-resource-actions"><button disabled={busy || !data.can_write} onClick={() => start(titleCommand(`legacy-gap-${g.id}`, '미확인 항목 수정', `legacy-gaps/${g.id}`, { expected_version: g.version }, g.title, g.reason, 'POST', 'edited_title', 'edited_reason'))}><Pencil size={13}/>수정</button><button disabled={busy || !data.can_write} onClick={() => start(noteCommand(`legacy-gap-${g.id}`, '확인 항목 제외', `legacy-gaps/${g.id}`, { expected_version: g.version, status: 'DISMISSED' }, 'POST'))}><X size={13}/>제외</button></div>{form(`legacy-gap-${g.id}`)}</article>)}
        <details className="completed-checklist"><summary>확인·제외 이력 {(data.gap_history ?? []).length}건</summary>{(data.gap_history ?? []).map((item, index) => <p key={`${item.entity_id}-${item.entity_version}-${index}`}><b>{gapHistoryLabel(item.operation)}</b> · {userText(item.after?.title || item.before?.title || '미확인 항목')} · {time(item.created_at)}{item.operation === 'EDIT' && item.before && <small>{userText(item.before.title)} → {userText(item.after.title)}<br/>{userText(item.before.reason)} → {userText(item.after.reason)}</small>}</p>)}</details>
      </section>
