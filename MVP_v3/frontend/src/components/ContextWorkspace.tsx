...
export const ContextWorkspace: React.FC<{ caseId: string; refreshKey: string; institutions: React.ReactNode; onOpenParticipants: () => void }> = ({ caseId, refreshKey, institutions, onOpenParticipants }) => {
  ...
  return <>
    ...
    {!data ? <p className="context-empty">사실·업무 기록을 불러오는 중입니다.</p> : <>
      {!data.can_review && <div className="permission-notice"><strong>검토 권한이 필요합니다</strong><p>사실 확정·업무 완료·판단 기록은 메인 담당자 또는 검토자가 처리할 수 있습니다.</p><button className="secondary-action" type="button" onClick={onOpenParticipants}>참여자 관리에서 내 역할 설정</button></div>}
...
<button className="secondary-action" disabled={busy || !data.can_write} onClick={() => start(titleCommand('fact-new', '사실 후보 추가', 'facts', { client_request_id: generateUuid(), semantic_key: `staff.observation.${generateUuid().replace(/-/g, '')}`, value: {} }, '', '', 'POST', 'display_label', 'display_value'))}><Plus size={13}/>사실 후보 추가</button>{form('fact-new')}
      </section>
