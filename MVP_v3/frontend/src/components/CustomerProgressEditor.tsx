...
export const CustomerProgressEditor: React.FC<{
  ...
  const edit = (item: CustomerProgressItem) => {
    if (editing?.step === item.step) {
      setEditing(null); setDraft(null); setError('');
      return;
    }
  ...
  return <section className="context-section public-progress-editor">
    ...
    <div className="public-progress-list">{items.map((item) => <article key={item.step} className={editing?.step === item.step ? 'is-editing' : ''}>
      ...
      {editing?.step === item.step && draft && <form className="form-grid progress-edit-form" onSubmit={(event) => void save(event)}>
