...
export const CaseAssignmentDialog: React.FC<Props> = ({ caseId, mode = 'initial', onAssigned, onSaved, onClose, initialRecommendation = false }) => {
  ...
  const matches = (person: Assignment) => { const needle = query.trim().toLowerCase(); return (roleFilter === 'ALL' || person.role_label === roleFilter) && (!needle || `${person.display_name} ${person.role_label} ${person.position_title ?? ''} ${person.status_text} ${person.assignment_eligible ? 'Case 배정 가능' : '자동 추천 제외'}`.toLowerCase().includes(needle)); };
  ...
  const toggle = (person: Assignment) => { if (person.isSelf) return; setSelected((current) => { const next = { ...current }; if (next[person.user_id]) delete next[person.user_id]; else next[person.user_id] = person; return next; }); };
