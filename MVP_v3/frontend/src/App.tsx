...
const Workspace: React.FC = () => {
  ...
  useEffect(() => { void loadCases(); const timer = window.setInterval(() => void loadCases(), 15000); return () => window.clearInterval(timer); }, [loadCases]);
  return <div className="app-shell">
    <header className="app-header"><div className="brand"><span><ShieldCheck size={19}/></span><div><b>CONTEXT-FIRST CASE</b><small>보이스피싱 대응 워크스페이스</small></div></div><div className={`header-status ${error ? 'has-error' : ''}`}><span><Wifi size={13}/>{error ? 'General API 연결 확인 필요' : 'General API 연결'}</span><button className="mobile-list-button" onClick={() => setMobileListOpen((value) => !value)}>{mobileListOpen ? <X size={17}/> : <List size={17}/>}사건 목록</button></div></header>
