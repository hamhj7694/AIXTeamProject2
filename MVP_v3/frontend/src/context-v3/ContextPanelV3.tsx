...
import { CustomerShareSection, CustomerSharedTaskResults, ExposureSection, FactVerificationSection, FraudCircumstanceSection, ImpersonationSection, StaffActionSection, SummarySection, visibleSummaryItems, type WorkflowAction } from './sections';
  ...
  return <aside className={`context-panel context-panel-v3 ${props.open ? 'is-open' : ''}`} aria-label="사건 맥락 V3">
    ...
    <div className="context-scroll" id="case-context-content">
      ...
      {visibleData && <Fragment key={props.caseItem.case_id}>
        ...
        <CustomerShareSection section={getSection('CUSTOMER_SHARE')} open={openSections.has('CUSTOMER_SHARE')} onOpenChange={(next) => setSectionOpen('CUSTOMER_SHARE', next)} progressEditor={<><CustomerSharedTaskResults section={getSection('CUSTOMER_SHARE')}/><CustomerProgressEditor caseId={props.caseItem.case_id} items={props.bundle.customer_progress ?? []} onSaved={props.onProgressSaved}/></>}/>
      </Fragment>}
    </div>
