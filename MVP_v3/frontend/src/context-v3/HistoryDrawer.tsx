...
export const HistoryDrawer: React.FC<{ open: boolean; events: CaseEvent[]; onClose: () => void }> = ({ open, events, onClose }) => {
  ...
  if (!open) return null;
  return <div className="context-history-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside ref={drawerRef} className="context-history-drawer" role="dialog" aria-modal="true" aria-labelledby="context-history-title" aria-describedby="context-history-description"><header><Clock3 size={18}/><div><h2 id="context-history-title">최근 사건 기록</h2><p id="context-history-description">사건의 주요 최근 활동을 확인할 수 있습니다.</p></div><button ref={closeRef} onClick={onClose} aria-label="최근 사건 기록 닫기"><X size={17}/></button></header><div>{events.length === 0 ? <p className="context-history-empty">표시할 최근 이벤트가 없습니다.</p> : events.map((event) => <article key={event.event_id}><header><strong>{eventLabel(event.event_type)}</strong><time>{new Date(event.occurred_at).toLocaleString('ko-KR')}</time></header><p>{actorLabel(event.actor_type)}</p></article>)}</div></aside></div>;
};
