...
export const ContextPanelFoundation: React.FC<Props> = ({ open, onToggle, caseItem, bundle, support, onOpenQuestions, onOpenTransactionLookup, onOpenVerification, onOpenAction, assigneeOptions = [], headerActions }) => {
  ...
  useEffect(() => {
    ...
    if (previousCount === 0 && inProgressCount > 0) {
      setGuideOpen((current) => ({ ...current, recommended: true }));
    }
  ...
  const guideCompletionGroup = (item: GuideEntry): 'incomplete' | 'completed' => {
    ...
    return item.stage === 'completed' ? 'completed' : 'incomplete';
  };
  const groupedGuides = (stage: GuideStage) => activeGuides.filter((item) => stage === 'completed' ? guideCompletionGroup(item) === 'completed' : guideCompletionGroup(item) === 'incomplete');
    ...
    return editingGuideId === item.id ? <form className="right-panel-inline-form" key={item.id} onSubmit={(event) => {
      ...
      <div className="right-panel-guide-copy">
        ...
        {(steps.length > 0 || archivedSteps.length > 0) && expanded && <div className="right-panel-guide-expanded" onClick={(event) => event.stopPropagation()}>
          {item.contextSummary && item.contextSummary.trim() !== item.description.trim() && <p className="right-panel-guide-context">{item.contextSummary}</p>}
