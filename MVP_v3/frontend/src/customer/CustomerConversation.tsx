...
const MessageEntry: React.FC<{ entry: CustomerTimelineEntry; message: CaseMessage; active: boolean; onToggle: Props['onToggleBookmark'] }> = ({ entry, message, active, onToggle }) => {
...
export const CustomerConversation: React.FC<Props> = ({ bundle, busy, bookmarkedIds, onAnswer, onRecoveryRequest, onToggleBookmark }) => {
  ...
  const renderEntry = (entry: CustomerTimelineEntry) => {
    ...
    if (entry.kind === 'MESSAGE') return <MessageEntry key={entry.id} entry={entry} message={entry.data as CaseMessage} active={active} onToggle={onToggleBookmark}/>;
