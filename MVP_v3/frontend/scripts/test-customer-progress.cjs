...
assert.equal((render(unknown.map((item) => ({ ...item, confirmation_requested: true }))).match(/담당자에게 확인 요청<\/button>/g) || []).length, 0, 'pending requests do not offer duplicate submission');
...
console.log('Customer progress render checks: 6 passed');
