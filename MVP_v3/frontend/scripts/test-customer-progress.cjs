...
assert.doesNotMatch(customerPageSource, /customer-details-toggle|detailsOpen|customer-side-scrim|customer-side-panel-header|PanelRightOpen|PanelRightClose/);
assert.doesNotMatch(styles, /customer-details-toggle|customer-side-scrim|customer-side-panel-header|customer-side-panel\.is-open/);
assert.match(styles, /@media \(max-width: 980px\)[\s\S]*\.customer-side-panel \{ width: 100%; overflow: visible; \}/);
...
console.log('Customer progress compact list checks: 36 passed');
