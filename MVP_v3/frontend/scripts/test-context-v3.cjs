...
assert.match(components, /open && item\.evidence_refs\.length > 0/);
...
assert.match(panel, /reviewContextFact\([^;]+decision === 'CONFIRM' \? '담당자 확인' : '잘못된 정보로 제외'\)/);
assert.doesNotMatch(panel, /supersedes_fact_id|replacingFactId/);
