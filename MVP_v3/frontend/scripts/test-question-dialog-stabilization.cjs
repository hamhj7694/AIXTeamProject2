...
const reconciled = reconcileQuestionDraft(current, ['old-transfer', 'stale-auth', 'staff-manual'], authoritative);
assert.deepEqual(Array.from(reconciled.items, item => item.question_id), ['old-transfer', 'staff-manual', 'server-personal']);
assert.equal(reconciled.items[0].question_text, '사용자가 편집한 송금 질문');
assert.deepEqual(Array.from(reconciled.selected), ['old-transfer', 'staff-manual', 'server-personal']);
