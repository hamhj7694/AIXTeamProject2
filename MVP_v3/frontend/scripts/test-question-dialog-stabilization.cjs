...
const { reconcileQuestionDraft, visibleQuestionCandidates } = moduleObject.exports;
...
assert.equal(visibleQuestionCandidates(sevenCandidates, false).length, 5);
assert.equal(visibleQuestionCandidates(sevenCandidates, true).length, 7);
assert.equal(visibleQuestionCandidates(sevenCandidates.slice(0, 2), false).length, 2);
...
assert.match(source, /추가 후보.*개 보기/);
assert.match(source, /items\.slice\(5\).*selected\.includes/);
