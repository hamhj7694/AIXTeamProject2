...
상위 기준 문서: `docs/03_IMPLEMENTATION_STATUS.md`
제품 기준 문서: `MVP_v3/PRD.md`, `MVP_v3/CUSTOMER_PRD.md`
최종 갱신: 2026-09-05
상태 표기: `[x] 완료`, `[-] 일부 완료·검증 제한`, `[ ] 후속`

이 문서는 세부 이력과 장기 백로그를 추적한다. 현재 완료 범위, 최신 검증 수치, 반드시 먼저 수행할 작업은 `03_IMPLEMENTATION_STATUS.md`를 단일 기준으로 사용한다. 두 문서가 다르면 실제 코드와 검증 결과를 확인한 뒤 `03_IMPLEMENTATION_STATUS.md`를 먼저 바로잡는다. 완료 표시는 코드와 정적·계약·필요 시 브라우저 검증 근거가 있는 항목에만 사용한다.

## 0. 현재 우선순위

> 2026-09-05 마감 범위 확정: Frontend·서버·사건 RAG 연결만 안정화한다. 로그인 체계, 대규모 리팩터링, SSE/새 벡터 DB 도입은 제외한다. 아래 장기 체크박스를 이번 마감 요구사항으로 확대하지 않는다. `11_DEADLINE_STABILIZATION.md` 참조.

- [x] 마감-1 초기 참여 등록 순서와 역할 변경 후 버튼 권한 갱신; 사용자 승인 MVP 모드에서 은행 사용자 편집·검토 권한 개방 및 불필요한 역할 경고 제거
- [x] 마감-2 같은 사건·공개 범위로 제한한 한국어 문구/동의어 RAG → 은행/고객 AI 연결
- [x] 마감-3 새 직원 사실·업무·결정 → 은행 AI·업무 카드·최종 보고서 입력 연결
- [x] 마감-4 General 138 / Frontend 34 및 모드별 렌더링·fallback 10 검사, production build 통과; AI 89는 미변경 직전 기준
- [x] 마감-5 서버 3개 HTTP 200, 실제 DB 검색 후 AI 1회 응답; 사건 쓰기 없음
- [ ] 마감-6 실제 브라우저에서 MVP 모드 버튼 클릭·저장·새로고침 유지 검증 (브라우저 연결 필요)
- [x] 마감-7 다른 PC README: 기본 스키마 이후 추가 migration 필수 단계, 환경설정·서버 실행·기존 DB 주의사항 보완
- [ ] 마감-8 별도 PC 최초 설치 실검증 (현재 PC의 테스트 통과와 구분)
- [x] 마감-9 사건 맥락 CSS 통일: 제목 위계·접기 제목·카드·폼·저장/취소 버튼·중첩 여백 정리; build/회귀 34개·정적 스타일 13개 통과. 실제 시각 QA는 마감-6에 남김
- [x] 표시-조사 영문·내부 코드 노출 관련 소스/계약 110개 정적 검색 및 핵심 경로 재현; `12_USER_TEXT_AUDIT.md` 작성 (구현 완료 아님)
- [x] 표시-최소수정 JSON/직원 원문 보호, 알려진 한글 용어·별칭/사전 동등성, 질문 선택지 라벨·AI 업무 초안·보고서 화면/출력 문자열·한국어 오류 안내 반영 (미등록 용어 무조건 치환은 제거)
- [x] 채팅-경합 은행/고객 전송의 사건·요청 세대 검증, 지연 성공/실패/예약 AI 콜백 회귀 검사
- [x] 답변-원자성 메시지·답변·사실 후보·접수 안내 동시 저장 및 재전송 멱등 처리, MySQL 동시 호출/rollback 검증
- [x] 종결-경로 일반 PATCH의 CLOSED 전환 차단, 기존 관리자 암호·최종 보고서 종결 유지
- [x] 최소수정-검증 General 145(+하위13), AI 89(+하위10), 공개 계약5, Frontend 기존34+보고서 SSR5+추가 표시/채팅 회귀, production build 통과
- [ ] 최소수정-실화면 실제 브라우저 편집/재저장·사건 전환·다운로드 시각 확인 (이번 변경에서 서버 재시작/유료 AI 호출 없음)
- [x] 고객질문-출처 현재 ASKED 서비스 카드의 공개 필드를 AI에 직접 전달하고 미발송/타 사건/내부 사유 제외. 서비스 사실 확인·실제 비밀값 요청·외부 요구·불명확 출처를 구분하는 프롬프트 및 단위 검사 추가
- [x] 고객질문-검증 General145/AI91 통과. 합성 gpt-4o-mini 응답에서 보완 후 서비스 응답 허용/비밀값 입력 거부/출처 되묻기 확인. General/AI 재시작·3개 서버 HTTP200 확인 (실제 사건 쓰기 없음)
- [x] 고객질문-안정화 현재 ASKED 사실 확인 카드의 응답 허용 안내는 결정론적으로 고정하고 외부 요구·민감값·실행 지시는 제외
- [x] 고객답변-의미투영 부분 개인정보 제공 표현을 구조화하고 투영 schema v3로 이전 캐시 무효화; 알림 보완 후 VP-8 CURRENT revision 48에서 표시 확인
- [x] 고객요청-가시성 확인 요청 완료 버튼 유지, 고객 채팅·은행 내부 채팅 revision 단위 멱등 알림 연결
- [x] AI역할-분리 고객 안전 상담/은행 CaseCopilot의 모델 환경변수와 Docker 전달 경로 분리; 기본 모델은 비용 평가 전 동일 유지

- [x] P0-0 사건 맥락 v2 목표 데이터 계약 문서화 (`09_CASE_CONTEXT_DATA_CONTRACT.md`)
- [x] P0-0a 계약 승인, Public/AI 계약, additive migration·rollback 계획 및 격리 DB 검증
- [x] P0-0b v2 저장 repository와 리소스별 General API 명령 구현
- [x] P0-0b-deploy 개발 DB `014` 적용 — General API 중지 후 적용·재기동 완료
- [x] P0-0d 새 workspace 조회와 사실·제안·업무·판단 분리 화면, 기존 기록 보존 조회 및 기존 제안 채택 연결
- [x] P0-0e-input 새 업무·결정의 AI 채팅/보고서 입력 통합
- [ ] P0-0e-sync 기존 고객 답변·확정 사실의 새 Fact/Gap 영속 동기화 (이번에 자동 이관하지 않음)
- [ ] P0-0c 기존 사건 맥락 경로와 병행하는 새 shadow projection 및 이관 검증
- [ ] P0-1 실제 브라우저 전체 연동 E2E: 분석 → Case 생성 → 양방향 채팅 → AI → 질문·답변 → 사건 맥락 → 새로고침 유지
- [ ] P0-2 `context_revision` 변경 시에만 실행되는 실제 LLM 사건 맥락 종합 요약
- [x] P0-3-lexical 구조화 상태 선검사 + 사건 기록 문구/동의어 검색 RAG와 보수적인 중복 문구 필터
- [ ] P0-3-semantic 임베딩 의미 검색 기반 중복 질문 방지 (후속 고도화)
- [ ] P0-4 실제 인증 세션과 Case 역할 기반 서버 RBAC
- [ ] P1-1 고객 진행 상황과 AI 답변의 확인 상태 일치 검증
- [ ] P1-2 polling을 SSE/WebSocket과 장애 시 fallback polling 구조로 전환
- [ ] P1-3 직원 편집·체크리스트 통제·동시 수정·이력의 실제 MySQL/브라우저 QA
- [ ] P1-4 `risk_score` 정밀도, MySQL 폐기 예정 구문, 내부 코드명 노출 경로 정리
...
- [x] E-14 고객 질문 발송 시 은행 내부 Timeline에 얇은 발송 안내 카드와 현재 답변 상태 표시
- [x] E-15 DB의 최신 FINAL 보고서를 기준으로 12영역 문서 카드 표시 및 과거 보고서 호환 처리
- [ ] E-16 인증 도입 후 메시지 본인 판정을 서버 세션 값으로 교체
- [ ] E-17 서버가 보장하는 전역 sequence로 동일 시각 항목의 순서를 확정
...
- [x] G-13 사건 종결 시 최신 Shared Case 기반 12영역 AI 보고서 생성·원자적 저장·PDF/Word 내보내기
