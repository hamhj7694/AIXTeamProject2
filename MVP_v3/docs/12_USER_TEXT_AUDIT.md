...
아래 1~5절은 수정 전 조사 근거로 보존한다. 사용자 승인 후 다음 최소 수정을 적용했다.

- U-01/U-07: REPORT_CARD/구조화 JSON content·ID·enum·직원 작성/편집 원문을 응답 가공에서 보호한다. 보고서는 파싱 후 표시 문자열만 변환하며 직원 편집 폼은 저장 원문을 사용한다. 미등록 snake_case 일괄 치환을 제거했다.
- U-02/U-03: Social Engineering의 공백/하이픈/밑줄 별칭을 한국어로 표시하고 초기 AI 요약의 모든 표시 문자열에 한국어 지침을 추가했다. 두 언어의 사전 전체 동등성·URL/파일명 보호를 회귀 검사한다. 기존 DB 재작성/AI 재분석은 하지 않았다.
- U-04/U-05: AI 업무 초안 표시 필드, 질문 선택지/답변 표시 라벨, JSON/정규 보고서 카드 및 PDF/Word 출력 문자열을 보완했다. 옵션 전송 값과 직원 종결 메모는 원문 보존. 현재 화면에 사용하지 않는 context_sources를 무조건 변환하지 않는다.
- U-06: 프론트 오류 code별 한국어 안내, 422 입력 검증 안내, fetch 연결 실패 안내를 적용했다. 기존 서버의 기술 오류 코드/계약은 유지한다.
- 별도 기능 수정: 양쪽 채팅의 사건 전환 경합 방어, 답변/메시지/사실 후보/접수 안내의 원자적 저장·재전송 중복 방지, 일반 PATCH 종결 우회 차단.

검증: General 145(+하위13), AI 89(+하위10), 공개 계약5 통과. Frontend build·기존 회귀34·보고서 응답→파싱→SSR5·추가 표시/채팅 경합 스크립트 통과. MySQL은 임시 격리 스키마에서 동시 답변·실패 rollback을 검증했다.

제한: 실제 브라우저 클릭/직원 재저장 E2E, PDF/Word 시각 확인, 유료 LLM 새 출력 검사는 미수행. 서버 재시작 및 실제 사건 데이터 수정은 하지 않았다. 알려지지 않은 영어를 전부 제거하지 않으며 구조와 원문 보존을 우선한다.
