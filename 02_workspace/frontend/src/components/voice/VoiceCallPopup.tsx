...
import { getVoiceCallSnapshot, updateVoiceCallSnapshot } from './voiceCallPersistence';
...
export const VoiceCallPopup: React.FC<VoiceCallPopupProps> = ({ open, role, onClose, onCallStarted, onCallEnded, onRecordingReady }) => {
  ...
  useEffect(() => {
    if (!calling) { setElapsedSeconds(0); return; }
    const callStartedAt = startedAt ?? Date.now();
    if (!startedAt) {
      setStartedAt(callStartedAt);
      updateVoiceCallSnapshot(role, { open: true, calling: true, startedAt: callStartedAt });
    }
    const updateElapsed = () => setElapsedSeconds(Math.max(0, Math.floor((Date.now() - callStartedAt) / 1000)));
    updateElapsed();
    const timer = window.setInterval(updateElapsed, 1000);
    return () => window.clearInterval(timer);
  }, [calling, role, startedAt]);
  ...
  const finishCall = () => {
    ...
    recorder.onstop = () => {
      ...
      updateVoiceCallSnapshot(role, { open: false, calling: false, startedAt: null });
      onClose();
    };
    recorder.stop();
  };
  ...
  return (
    <div ref={popupRef} style={position ? { left: position.x, top: position.y } : undefined} className={`fixed z-50 h-fit min-h-[240px] w-80 min-w-[280px] max-w-[calc(100vw-2rem)] resize overflow-auto rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/20 ${position ? '' : 'left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2'}`}>
      <div onPointerDown={handleDragStart} onPointerMove={handleDrag} className="flex cursor-move touch-none items-center justify-between border-b border-slate-100 bg-slate-900 px-4 py-3 text-white">
        <div className="flex items-center gap-2"><span className={`h-2.5 w-2.5 rounded-full ${calling ? 'animate-pulse bg-emerald-400' : 'bg-amber-400'}`} /><span className="text-xs font-bold">{calling ? `통화 중 ${String(Math.floor(elapsedSeconds / 60)).padStart(2, '0')}:${String(elapsedSeconds % 60).padStart(2, '0')}` : '음성 통화'}</span></div>
