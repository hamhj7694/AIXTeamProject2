...
class InMemoryCaseRepository:
    ...
    async def append_message(self, case_id: str, record: dict[str, Any]) -> dict[str, Any]:
        async with self._lock:
            ...
            message = {"message_id": f"msg-{uuid4().hex}", "case_id": case_id, **record, "created_at": now}
            self._messages.append(message)
            self._events.append({
                "event_id": len(self._events) + 1, "case_id": case_id, "event_type": "MESSAGE_ADDED",
                "actor_type": message["actor_type"], "payload": {"message_id": message["message_id"]}, "occurred_at": now,
            })
            return deepcopy(message)

    async def list_messages(self, case_id: str) -> list[dict[str, Any]]:
        return [deepcopy(item) for item in self._messages if item["case_id"] == case_id]
