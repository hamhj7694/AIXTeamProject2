...
class MySqlCaseRepository:
            ...
            try:
                async with connection.cursor() as cursor:
                    ...
                    if attachment_ids:
                        ...
                        await cursor.execute(
                            f"UPDATE attachments SET status='LINKED', message_id=%s WHERE case_id=%s AND attachment_id IN ({placeholders})",
                            (message_id, case_id, *attachment_ids),
                        )
                ...
                await connection.rollback()
                raise
        linked_attachments = await self.list_attachments(case_id)
        return {
            ...
            "attachments": [item for item in linked_attachments if item.get("message_id") == message_id],
    ...
    async def list_messages(self, case_id: str, channel: str | None = None) -> list[dict[str, Any]]:
        ...
        async with pool.acquire() as connection, connection.cursor(aiomysql.DictCursor) as cursor:
            ...
            return [
                {**row, "mentions": self._json(row["mentions_json"]) or [], "attachment_ids": self._json(row.get("attachments_json")) or [], "attachments": [], "created_at": row["created_at"].isoformat()}
                for row in await cursor.fetchall()
            ]
