...
export const CaseEntryPageV2: React.FC = () => {
  ...
  useEffect(() => {
    Promise.all([caseApi.get(caseId), mvpChatApi.listPresence(caseId)])
      .then(([caseDetail, presence]) => {
        setItem(caseDetail);
        const customer = presence.find((item) => item.channel === 'CUSTOMER' && (item.presence === 'VIEWING' || item.presence === 'TYPING'));
        setCustomerStatus(customer ? '상담 중' : caseDetail.transferred === null ? '대기중' : '부재중');
      })
      .catch((reason) => setError(reason instanceof Error ? reason.message : 'Case를 불러오지 못했습니다.'));
  }, [caseId]);
  useEffect(() => {
  ...
  return <AppLayout><main className="mx-auto max-w-6xl py-8 lg:ml-64"><Link to="/cases" className="mb-5 inline-flex items-center gap-1 text-sm font-bold text-slate-500"><ArrowLeft size={16}/> 보이스피싱 Case 목록</Link>
    ...
    <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-bold text-blue-600">AI 초기 판단</p><p className="mt-2 text-sm leading-7 text-slate-700">{item.aiInitialBrief}</p></section>
