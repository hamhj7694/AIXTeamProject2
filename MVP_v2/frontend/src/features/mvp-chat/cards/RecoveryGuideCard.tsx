...
/** Persistent compact recovery menu. Detailed procedures render below the chat workspace. */
export const RecoveryGuideCard: React.FC<Props> = ({ selectedStepId, onSelectStep }) => (
  <section className="rounded-2xl border border-rose-200 bg-gradient-to-br from-rose-50 to-white p-4 shadow-sm">
    ...
    <p className="mt-2 text-[11px] leading-5 text-slate-600">필요한 단계를 선택하면 화면 아래에 상세 절차가 열립니다.</p>
    <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
      {recoverySteps.map(({ id, icon: Icon, title, summary }) => <button type="button" key={id} onClick={() => { onSelectStep(id); window.setTimeout(() => document.getElementById('customer-recovery-detail')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50); }} className={`rounded-xl border p-3 text-left transition ${selectedStepId === id ? 'border-slate-900 bg-white ring-2 ring-slate-900' : 'border-rose-100 bg-white hover:border-rose-300'}`}>
