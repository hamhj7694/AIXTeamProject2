...
export const CaseFactsPanel: React.FC<{ caseId: string; compact?: boolean }> = ({ caseId, compact = false }) => {
  ...
  const load = useCallback(() => mvpChatApi.listCaseFacts(caseId).then(setFacts).catch((reason) => setError(reason instanceof Error ? reason.message : '확인 정보를 불러오지 못했습니다.')).finally(() => setLoading(false)), [caseId]);
