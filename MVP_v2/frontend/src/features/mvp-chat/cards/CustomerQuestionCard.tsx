...
export const CustomerQuestionCard: React.FC<Props> = ({ question, submitting = false, onSubmit }) => {
  ...
  const explanation = question.customer_explanation || (question.priority === 'P0'
    ? '안전을 위해 먼저 확인해야 하는 내용입니다. 기억나는 범위에서 선택해 주세요.'
    : '현재 상황에 맞는 다음 안내를 위해 확인하는 질문입니다.');
  ...
  return <section className="rounded-2xl border border-blue-200 bg-blue-50 p-4 shadow-sm">
    <div className="flex items-center gap-2 text-blue-800"><MessageCircleQuestion size={17}/><p className="text-xs font-black">안전 확인 질문 {question.sequence ? `· ${question.sequence}번` : ''}</p></div>
    <p className="mt-2 text-sm font-black leading-6 text-slate-900">{question.question_text}</p>
    <p className="mt-1 rounded-xl bg-white/80 px-3 py-2 text-xs leading-5 text-slate-600">{explanation}</p>
