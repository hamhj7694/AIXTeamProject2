...
export const CaseWorkflowSummaryPanel: React.FC<{ caseId: string }> = ({ caseId }) => {
  ...
  const items = [
    ...
    { icon: Building2, label: '대응 모드', value: mode === 'RECOVERY' ? '피해구제 진행' : mode === 'CLOSED' ? '처리 완료' : '피해 예방 진행', detail: '은행 대응 화면에서 다음 업무 확인', to: `/cases/${caseId}/bank`, color: mode === 'RECOVERY' ? 'text-rose-700 bg-rose-50' : 'text-slate-700 bg-slate-100' },
  ];
