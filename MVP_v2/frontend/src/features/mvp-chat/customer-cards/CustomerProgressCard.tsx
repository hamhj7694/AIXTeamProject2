...
export const CustomerProgressCard: React.FC<Props> = ({ currentCase, questions, verificationCount, recoveryActive = false }) => {
  ...
  const preventSteps = [
    ...
    { label: '필요 정보 확인', done: waiting === 0 && answered > 0, active: waiting > 0 || answered === 0 },
    { label: '기관 확인', done: verificationCount > 0 || status === 'IN_PROGRESS' || status === 'CLOSED', active: status === 'VERIFYING' },
    { label: '보호 조치', done: status === 'CLOSED', active: status === 'IN_PROGRESS' },
    { label: '처리 완료', done: status === 'CLOSED', active: false },
  ];
  const recoverySteps = [
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
    ...
    <p className="mt-4 rounded-xl bg-slate-50 p-3 text-[11px] leading-5 text-slate-600">{recovery ? '각 단계는 고객 요청과 은행 담당자의 처리 결과에 따라 갱신됩니다.' : `답변 완료 ${answered}건 · 확인 대기 ${waiting}건${verificationCount > 0 ? ` · 공개 확인 결과 ${verificationCount}건` : ''}`}</p>
  </section>;
};
