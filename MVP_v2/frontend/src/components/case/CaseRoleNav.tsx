...
export const CaseRoleNav: React.FC<CaseRoleNavProps> = ({ caseId, compact = false }) => {
  ...
  return <nav aria-label="Case 역할 화면" className={compact ? 'grid w-full grid-cols-3 gap-1 border-t border-slate-200 pt-2 lg:grid-cols-1' : 'mb-6 flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm'}>
    {items.map(({ key, label, icon: Icon }) => {
      const path = `/cases/${caseId}/${key}`;
