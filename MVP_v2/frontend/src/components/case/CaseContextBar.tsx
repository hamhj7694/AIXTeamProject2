...
import { actualLossLabel, caseDisplayId, caseProgressMessage, caseProgressStage, victimStatusLabel, workflowStatusLabel } from '../../utils/casePresentation';
...
export const CaseContextBar: React.FC<{ item: CaseRecord; compact?: boolean; showProgress?: boolean }> = ({ item, compact = false, showProgress = false }) => {
  ...
  const currentStage = caseProgressStage(item.status, item.transferred);
  return (
